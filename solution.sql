...
VALUES
    ...
    (104, 3, 400.00),
    (105, 2, 250.00);
-- find the customer IDs and the average total amount of their orders. Display only those customers whose average total amount is greater than or equal to 250
...
-- retrieve the student name, course name, and credits for all enrolled courses.
...
-- retrieve the department name and the average salary of employees working in each department. Sort the results by the average salary in descending order.
...
-- find the total sales amount for each month, along with the number of orders in that month
...
-- retrieve the department name and the average salary of employees in each department, excluding departments with fewer than two employees.
...
-- find the product names and their total sales quantity for products with a total sales quantity greater than 5
...
-- retrieve the customer name, order date, and the total quantity of items ordered for each order.
...
-- find the total sales amount for each product. However, if the total sales amount is less than 300, show it as 0
...
-- find the minimum, maximum, and average age of all students.
